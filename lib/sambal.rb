  ...
  class Client
    ...
    def initialize(options={})
      begin
        ...
        smb_str = "smbclient //#{options[:host]}/#{options[:share]} -U #{options[:user]}%#{options[:password]} -p #{options[:port]}"
#        smb_str = "smbclient //#{options[:host]}/#{options[:share]} #{options[:password]} -W #{options[:domain]} -U #{options[:user]} -p #{options[:port]}"
        # puts "smb_str #{smb_str}"
        @o, @i, @pid = PTY.spawn(smb_str)
